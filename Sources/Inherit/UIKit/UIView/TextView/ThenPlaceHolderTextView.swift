...
open class ThenPlaceHolderTextView: UITextView {
    ...
    private lazy
    var placeholderLayoutManager: NSLayoutManager = NSLayoutManager()
    
    private lazy
    var placeholderTextContainer: NSTextContainer = NSTextContainer()
    ...
    @IBInspectable open
    var placeholderColor: UIColor = UIColor(red: 0.6, green: 0.6, blue: 0.6, alpha: 1.0) {
        didSet {
    ...
    open override
    var attributedText: NSAttributedString! { didSet { self.setNeedsDisplay() } }
    
    open override
    var bounds: CGRect { didSet { self.setNeedsDisplay() } }
    
    open override
    var contentInset: UIEdgeInsets { didSet { self.setNeedsDisplay() } }
    ...
    open override
    var textContainerInset: UIEdgeInsets { didSet { self.setNeedsDisplay() } }
    ...
    func caretRect(for position: UITextPosition) -> CGRect {
        ...
        guard self.text.isEmpty == true, let attributedPlaceholder = self.attributedPlaceholder, attributedPlaceholder.length > 0 else {
    ...
    func sizeThatFits(_ size: CGSize) -> CGSize {
        ...
        if self.isEmpty == true, let attributedPlaceholder = self.attributedPlaceholder, attributedPlaceholder.length > 0 {
            let placeholderInsets = self.placeholderInsets
    ...
    func sizeToFit() {
        if self.isEmpty == true, let attributedPlaceholder = self.attributedPlaceholder, attributedPlaceholder.length > 0 {
            let placeholderUsedRect = self.placeholderUsedRect(attributedPlaceholder: attributedPlaceholder, textContainerSize: .zero)
            let placeholderInsets = self.placeholderInsets
...
// MARK: - Private API
extension ThenPlaceHolderTextView {
    
    private func commonInitializer() {
    
        self.contentMode = .topLeft
        NotificationCenter.default.addObserver(self, selector: #selector(ThenPlaceHolderTextView.handleTextViewTextDidChangeNotification(_:)), name: UITextView.textDidChangeNotification, object: self)
    }
    
    @objc private func handleTextViewTextDidChangeNotification(_ notification: Notification) {
        guard let object = notification.object as? ThenPlaceHolderTextView, object === self else {
            return
        }
        self.setNeedsDisplay()
    }
