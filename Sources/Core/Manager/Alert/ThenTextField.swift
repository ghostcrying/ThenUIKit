...
open class ThenTextField: UITextField {
    
    public typealias Config = (ThenTextField) -> Swift.Void
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        return bounds.insetBy(dx: (leftTextPadding ?? 8) + (leftView?.bounds.width ?? 0) + (leftViewPadding ?? 0), dy: 0)
    }
    ...
    public func action(closure: @escaping Action) {
        if actionEditingChanged == nil {
            addTarget(self, action: #selector(ThenTextField.textFieldDidChange), for: .editingChanged)
        }
