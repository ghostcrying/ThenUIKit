...
//  ThenUIKit
//
//  Created by ghost on 2018/10/12.
//
...
public class QRScanner: NSObject, AVCaptureMetadataOutputObjectsDelegate {
    ...
    private let _rectOfInterest: CGRect
    ...
    public init(rectOfInterest: CGRect = CGRect(x: 0, y: 0, width: 1, height: 1), _ handle: ((String) -> Void)?) {
        ...
        lock.then.lock(_isScanning = false)
    ...
    func setup(_ completion: ((Bool) -> ())? = nil) {
        ...
        DispatchQueue.global().async { [weak self] in
            ...
            captureMetadataOutput.rectOfInterest = self._rectOfInterest
...
public class QRScannerAnimationView: UIView {
    ...
    public override func layoutSubviews() {
        ...
        focusContentImageView.then.layout {
            $0.size = _focusSize
    ...
    private func updateShapePath() {
        let toLeftRight = ((bounds.width - _focusSize.width) / 2).then.floor
        let toTopBottom = ((bounds.height - _focusSize.height) / 2).then.floor
