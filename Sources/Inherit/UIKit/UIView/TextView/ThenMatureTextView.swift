...
/// The `ThenMatureTextViewDelegate` protocol extends the `UITextViewDelegate` protocol by providing a set of optional methods you can use to receive messages related to the change of the height of `ThenMatureTextView` objects.
@objc public protocol ThenMatureTextViewDelegate: UITextViewDelegate {
    ///
    ...
    ///     - matureTextViewHeightBegin: CGFloat that identifies the start height of the growing text view.
    ///     - matureTextViewHeightEnd: CGFloat that identifies the end height of the growing text view.
    ///
    @objc optional func matureTextView(_ textView: ThenMatureTextView, didChangeHeightFrom matureTextViewHeightBegin: CGFloat, to matureTextViewHeightEnd: CGFloat)
    ...
    ///     - matureTextViewHeightEnd: CGFloat that identifies the end height of the growing text view.
    ///
    @objc optional func matureTextView(_ textView: ThenMatureTextView, willChangeHeightFrom matureTextViewHeightBegin: CGFloat, to matureTextViewHeightEnd: CGFloat)
}
...
@IBDesignable open class ThenMatureTextView: ThenPlaceHolderTextView {
    ...
    @IBInspectable open
    var animateHeightChange: Bool = true
    
    /// The receiver's delegate.
    @objc open weak
    var matureTextViewDelegate: ThenMatureTextViewDelegate? { didSet { delegate = matureTextViewDelegate } }
    
    /// The duration of the animation of the height change. The default value is `0.35`.
    @IBInspectable open
    var heightChangeAnimationDuration: Double = 0.35
    ...
    // MARK: - Actions
    @objc private
    func handleThenMatureTextViewTextDidChangeNotification(_ notification: Notification) {
        refreshHeightIfNeededAnimated(animateHeightChange)
    }
    ...
    private func commonInitializer() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(ThenMatureTextView.handleThenMatureTextViewTextDidChangeNotification(_:)), name: UITextView.textDidChangeNotification, object: self)
    }
    ...
    private func refreshHeightIfNeededAnimated(_ animated: Bool) {
        ...
        if oldHeight != newHeight {
            ...
            let heightChangeCompletionBlock: HeightChangeCompletionBlockType = { (oldHeight: CGFloat, newHeight: CGFloat) -> Void in
                ...
                self.matureTextViewDelegate?.matureTextView?(self, didChangeHeightFrom: oldHeight, to: newHeight)
            }
            matureTextViewDelegate?.matureTextView?(self, willChangeHeightFrom: oldHeight, to: newHeight)
            if animated {
                UIView.animate(
